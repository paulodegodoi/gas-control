...
export default function App() {
	...
	const currentReadings = useMemo(() => {
		return activeApartments.map(apt => {
			...
			r => r.apartmentId === apt.id && r.month === selectedMonth
			);
					...
					<div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-3 bg-slate-50 p-2 rounded-xl border border-slate-100">
						<div className="flex items-center space-x-2">
							...
							<input
								...
								onBlur={(e) => saveGasPrice(parseFloat(e.target.value) || 0)}
								className="w-24 bg-white border border-slate-200 rounded-lg px-3 py-2 font-medium text-slate-700 outline-none focus:ring-2 focus:ring-primary-500 transition-all"
							/>
						</div>
												...
												<div className="flex items-center space-x-6">
													...
													<div className="w-28 text-right bg-slate-50 p-2 rounded-lg border border-slate-100">
														...
														<span className={`text-lg font-bold ${consumo > 0 ? 'text-emerald-600' : 'text-slate-400'}`}>
															R$ {consumo > 0 ? (consumo * gasPrice).toFixed(2) : '0.00'}
														</span>
													</div>
												</div>
											</div>
