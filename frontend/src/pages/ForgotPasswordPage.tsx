...
export default function ForgotPasswordPage() {
	...
	const handleSubmit = async (e: FormEvent) => {
		...
		try {
			...
			if (!res.ok) {
				setError(data.message || data || 'Erro ao solicitar recuperação.');
			} else {
				setMessage(data.message || 'E-mail enviado se o cadastro existir.');
			}
		} catch (err) {
			...
			<div className="login-card">
				...
				<form onSubmit={handleSubmit} className="login-form" noValidate>
					...
					<button
						...
						disabled={loading}
					>
						{loading ? <span className="login-spinner" /> : 'Enviar link de recuperação'}
					</button>

					<div style={{ marginTop: '1rem', textAlign: 'center' }}>
						<Link to="/login" style={{ color: '#fff', fontSize: '0.9rem', textDecoration: 'underline' }}>
							Voltar para o Login
						</Link>
					</div>
				</form>
