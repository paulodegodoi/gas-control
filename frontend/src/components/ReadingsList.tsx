...
export default function ReadingsList({
    ...
    const handleSelectInput = (apartmentId: string) => {
        if (readOnly) return;
        // select input with id apartmentId
        const input = document.getElementById(`input-${apartmentId}`);
        if (input) {
            input.focus();
        }
    };
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                <h2 className="text-xl font-semibold text-slate-800">
                    Leituras de {selectedMonth}
                </h2>
            </div>
            <div className="divide-y divide-slate-100">
                {currentReadingsData.length === 0 ? (
                    <div className="p-8 text-center text-slate-500">
                        Nenhum apartamento ativo para registro.
                    </div>
                ) : (
                    currentReadingsData.map(
                        ({
                            apartment,
                            previousReading,
                            ...
                            >
                                ...
                                <div className="flex items-center space-x-4 lg:space-x-8 w-full lg:w-auto justify-end">
                                    <div className="flex flex-col items-end">
                                        <label className="text-xs text-slate-500 mb-1 font-medium whitespace-nowrap">
                                            Leitura Atual (m³)
                                        </label>
                                        {readOnly ? (
                                    ...
                                    <div className="w-24 text-right">
                                        ...
                                        <span
                                            className={`text-lg font-bold ${consumo > 0 ? "text-primary-600" : "text-slate-400"}`}
                                        >
                                            {consumo > 0
                                                ? consumo.toFixed(2)
                                                : "0.00"}
