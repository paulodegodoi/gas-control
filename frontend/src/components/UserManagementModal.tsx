...
export default function UserManagementModal({ isOpen, onClose }: UserManagementModalProps) {
    ...
    useEffect(() => {
        ...
        const loadContent = async () => {
            try {
                const resCondos = await fetch(`${API_BASE}/api/condominiums`, {
    ...
    useEffect(() => {
        if (!isOpen || !token || !selectedCondominium || role !== 'Morador') return;

        const loadApts = async () => {
            try {
                const resApt = await fetch(`${API_BASE}/api/apartments?condominiumId=${selectedCondominium}`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                if (resApt.ok) {
        ...
        loadApts();
    }, [selectedCondominium, role, isOpen, token]);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            <div className="bg-white rounded-3xl shadow-xl w-full max-w-lg p-8 transform scale-100 opacity-100 transition-all">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-black text-slate-800">Novo Usuário</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-1">Tipo de Perfil</label>
                            <select value={role} onChange={e => setRole(e.target.value as UserRole)}
                                ...
                                {user?.role === 'Admin' && <option value="Sindico">Síndico do Prédio</option>}
                            </select>
                        </div>
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-1">Condomínio</label>
                            <select value={selectedCondominium} onChange={e => setSelectedCondominium(e.target.value)} required
                                className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 outline-none">
                                {condominiums.map(c => (
                    ...
                    {role === 'Morador' && (
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-1">Vincular Apartamento</label>
                            <select value={selectedApartment} onChange={e => setSelectedApartment(e.target.value)} required
                                className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 outline-none">
                                {apartments.length === 0 && <option value="">Nenhum apt disponível...</option>}
                                {apartments.map(a => (
                                    <option key={a.id} value={a.id}>{a.number} - {a.name}</option>
                                ))}
                            </select>
                            {apartments.length === 0 && selectedCondominium && (
                                <p className="text-xs text-amber-600 mt-1">Este condomínio não possui apartamentos. Crie-os primeiro.</p>
                            )}
                        </div>
                    )}
