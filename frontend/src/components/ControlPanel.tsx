...
export default function ControlPanel({
    ...
    const [selectedMonth, setSelectedMonth] = useState<string>(() => {
        const saved = localStorage.getItem(
            `${moduleName}Control_selectedMonth`,
        );
    ...
    const [readingsInEdit, setReadingsInEdit] = useState<
    ...
    const readingsApiUrl =
        moduleName === "gas" ? "gas/readings" : "water/readings";
    ...
    const fetchReadingsForMonth = useCallback(
        async (month: string) => {
    ...
    const fetchPrice = useCallback(
        async (month: string) => {
            ...
            try {
                const url =
                    `${API_BASE}/api/${priceApiUrl}/${month}` +
                    (activeCondominiumId
    ...
    const handleUpdateReadingInput = (
        apartmentId: string,
        valueStr: string,
    ) => {
    ...
    const saveAllEdits = async () => {
        ...
        const requests = Object.entries(readingsInEdit).map(
            ([aptId, valStr]) => {
    ...
    const currentReadingsData = useMemo<CurrentReadingData[]>(() => {
        return activeApartments.map((apt) => {
            const prevReadingObj = previousMonthReadings.find(
                (r) => r.apartmentId === apt.id,
            );
            const currentReadingObj = currentMonthReadings.find(
                (r) => r.apartmentId === apt.id,
            );

            const previousReading = prevReadingObj
                ? prevReadingObj.currentReading
                : 0;
            const dbCurrentReading = currentReadingObj
                ? currentReadingObj.currentReading
                : previousReading;
    ...
    const { totalConsumption, totalToPay, effectivePrice } = useMemo(() => {
        ...
        let pay = 0;
        const overridePrice =
            priceInEdit !== null ? parseFloat(priceInEdit) || 0 : price;
    ...
    return (
        <div className="space-y-6">
            <ControlHeader
                ...
                subtitle={
                    moduleName === "gas"
                        ? "Sistema de gestão de consumo de gás"
                        : "Sistema de gestão de consumo de água"
                }
                price={price}
