...
const CATEGORIAS = ["Pessoal", "Consumo", "Manutenção", "Material", "Seguros", "Administrativo", "Fundo de Reserva"];
...
export default function FinancialDashboard() {
    ...
    const [deleteConfirmData, setDeleteConfirmData] = useState<{ catName: string; subId: string } | null>(null);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            setIsLoading(true);
            try {
                const res = await fetch(`${API_BASE}/api/finance/${activeCondominiumId}/${year}`, {
                ...
                if (res.ok) {
                    ...
                    CATEGORIAS.forEach(catName => {
                        const found = fetchedData.find(c => c.name === catName);
                        if (found) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE}/api/finance/sync/${activeCondominiumId}/${year}`, {
                method: 'POST',
                headers: { 
    ...
    const calculateTotalAnualBase = (cat: string) => data[cat]?.baseValues.reduce((acc, curr) => acc + curr, 0) || 0;
    const calculateTotalAnualSubCat = (sub: SubCategory) => sub.values.reduce((acc, curr) => acc + curr, 0);
    
    const calculateTotalAnualCategory = (cat: string) => {
        if (!data[cat]) return 0;
        return calculateTotalAnualBase(cat) + data[cat].subCategories.reduce((acc, sub) => acc + calculateTotalAnualSubCat(sub), 0);
    };

    // Summary data
    const totalGeralAno = CATEGORIAS.reduce((acc, cat) => acc + calculateTotalAnualCategory(cat), 0);
    ...
    const openSubCatModal = (parentCat: string, editSubId: string | null = null, currentName: string = "") => {
    ...
    const confirmDeleteSubCategory = () => {
        ...
        setData(prev => {
            const newData = { ...prev };
            const newSubs = newData[catName].subCategories.filter(s => s.id !== subId);
    ...
    const confirmAddSubCategory = (e: React.FormEvent) => {
        ...
        setData(prev => {
            ...
            if (activeEditSubId) {
                // Edit
                const subIndex = subsToKeep.findIndex(s => s.id === activeEditSubId);
                if (subIndex > -1) {
                    subsToKeep[subIndex] = { ...subsToKeep[subIndex], name: newSubCatName.trim() };
                }
            } else {
                // Add
                const hasExistingBaseValues = catData.baseValues.some(v => v > 0);
    ...
    const handleBaseValueChange = (catName: string, monthIdx: number, value: number) => {
        setData(prev => {
            const newData = { ...prev };
    ...
    const handleSubValueChange = (catName: string, subId: string, monthIdx: number, value: number) => {
        setData(prev => {
            const newData = { ...prev };
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500">Acompanhamento e previsão de despesas</p>
                </div>
                        ...
                        {isYearDropdownOpen && (
                            <>
                                ...
                                <div className="absolute right-0 mt-2 w-32 bg-white rounded-xl shadow-lg border border-slate-100 z-50 overflow-hidden py-1 max-h-64 overflow-y-auto hide-scrollbar">
                                    {[...Array(15)].map((_, i) => {
                                        const y = new Date().getFullYear() - 5 + i; // spanning -5 years up to +9 years
                                        return (
                                            <button
                                                key={y}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex items-center space-x-4">
                    ...
                    <div>
                        <div className="text-sm font-medium text-slate-500">Gasto Previsto Ano</div>
                        <div className="text-2xl font-bold text-slate-800">{formatCurrency(totalGeralAno)}</div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex items-center space-x-4">
                    ...
                    <div>
                        <div className="text-sm font-medium text-slate-500">Média Mensal Global</div>
                        <div className="text-2xl font-bold text-slate-800">{formatCurrency(mediaMensalTotal)}</div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex items-center space-x-4">
                    ...
                    <div>
                        <div className="text-sm font-medium text-slate-500">Maior Custo Mensal</div>
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex items-center space-x-4">
                    ...
                    <div>
                        <div className="text-sm font-medium text-slate-500">Categoria Principal</div>
                        <div className="text-2xl font-bold text-slate-800 truncate max-w-[120px]" title={CATEGORIAS.reduce((prev, current) => (calculateTotalAnualCategory(prev) > calculateTotalAnualCategory(current)) ? prev : current)}>
                            {CATEGORIAS.reduce((prev, current) => (calculateTotalAnualCategory(prev) > calculateTotalAnualCategory(current)) ? prev : current)}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
                <div className="p-6 border-b border-slate-100">
                    <h2 className="text-xl font-bold text-slate-800">Despesas por Categoria</h2>
                    <p className="text-sm text-slate-500 mt-1">Preencha as células para definir os custos previstos do ano.</p>
                </div>
                    ...
                    <table className="w-full text-left border-collapse min-w-[max-content]">
                        <thead>
                            <tr>
                                ...
                                {MESES.map(mes => (
                                    <th key={mes} className="py-4 px-4 bg-slate-50 text-slate-600 font-semibold text-sm uppercase tracking-wider border-b border-slate-200 text-right">
                                        {mes}
                                    </th>
                                ))}
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {CATEGORIAS.map((cat, idx) => {
                                ...
                                const hasSubCats = catData.subCategories.length > 0;
                                
                                return (
                                    <Fragment key={cat}>
                                        <tr className="group hover:bg-slate-50/80 transition-colors">
                                            <td className="py-2 px-6 text-sm font-medium text-slate-800 border-r border-slate-50 bg-white group-hover:bg-slate-50/80 sticky left-0 shadow-[2px_0_4px_-2px_rgba(0,0,0,0.05)] z-10 w-64">
                                                <div className="flex items-center justify-between">
                                                    <div 
                                                        className={`flex items-center space-x-3 select-none ${hasSubCats ? 'cursor-pointer' : ''}`} 
                                                        onClick={() => hasSubCats && toggleExpand(cat)}
                                                    >
                                                        <div className={`w-3 h-3 rounded-full shrink-0
                                                            ${idx % 7 === 0 ? 'bg-blue-500' : 
                                                              idx % 7 === 1 ? 'bg-indigo-500' : 
                                                              idx % 7 === 2 ? 'bg-emerald-500' : 
                                                              idx % 7 === 3 ? 'bg-amber-500' : 
                                                              idx % 7 === 4 ? 'bg-rose-500' : 
                                                              idx % 7 === 5 ? 'bg-purple-500' : 'bg-cyan-500'}`} 
                                                        />
                                                        <span className="whitespace-nowrap flex items-center">
                                                            {cat}
                                                            {hasSubCats && (
                                                                isExpanded ? <ChevronDown className="w-4 h-4 ml-1 text-slate-400" /> : <ChevronRight className="w-4 h-4 ml-1 text-slate-400" />
                                                            )}
                                                        </span>
                                                    </div>
                                                    <button 
                                                        onClick={(e) => { e.stopPropagation(); openSubCatModal(cat); }}
                                            ...
                                            <td className="py-4 px-6 text-sm font-bold text-slate-800 text-right bg-blue-50/30 group-hover:bg-blue-50/70 transition-colors">
                                                {formatCurrency(calculateTotalAnualCategory(cat))}
                                            </td>
                                        </tr>

                                        {isExpanded && catData.subCategories.map(sub => (
                                            <tr key={sub.id} className="group/sub hover:bg-slate-50/80 transition-colors bg-slate-50/30">
                                                <td className="py-2 px-6 pl-10 text-sm text-slate-600 border-r border-slate-50 bg-slate-50/30 group-hover/sub:bg-slate-50/80 sticky left-0 shadow-[2px_0_4px_-2px_rgba(0,0,0,0.05)] z-10 w-64">
                                                    <div className="flex items-center justify-between text-slate-500 font-medium w-full">
                                                        ...
                                                        <div className="flex items-center opacity-0 group-hover/sub:opacity-100 transition-opacity shrink-0 space-x-1">
                                                            <button 
                                                                onClick={(e) => { e.stopPropagation(); openSubCatModal(cat, sub.id, sub.name); }}
                                                            ...
                                                            <button 
                                                                onClick={(e) => { e.stopPropagation(); requestDeleteSubCategory(cat, sub.id); }}
                                                ...
                                                <td className="py-3 px-6 text-sm font-semibold text-slate-700 text-right bg-blue-50/10">
                                                    {formatCurrency(calculateTotalAnualSubCat(sub))}
                                                </td>
                                            </tr>
                                        ))}
                                    </Fragment>
                        ...
                        <tfoot>
                            <tr className="bg-slate-800 text-white">
                                ...
                                {Array.from({length: 12}).map((_, idx) => {
                                    const totalMes = CATEGORIAS.reduce((acc, cat) => {
                                        if (!data[cat]) return acc;
                                        return acc + data[cat].baseValues[idx] + data[cat].subCategories.reduce((subAcc, sub) => subAcc + sub.values[idx], 0);
                                    }, 0);
                                    return (
                                        <td key={idx} className="py-4 px-4 text-sm font-bold text-right text-slate-200">
                ...
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                            <h3 className="font-bold text-slate-800">{activeEditSubId ? "Renomear Subcategoria" : "Nova Subcategoria"}</h3>
                            <button 
                        ...
                        <form onSubmit={confirmAddSubCategory}>
                            <div className="px-6 py-6 space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1">
                                        Categoria Pai: <span className="font-bold text-blue-600">{activeParentCat}</span>
                                    </label>
                                    <input 
                                        type="text" 
                                        autoFocus
                            ...
                            <div className="px-6 py-4 bg-slate-50 flex justify-end space-x-3">
                                ...
                                >
                                    {activeEditSubId ? "Salvar Alteração" : "Adicionar"}
                                </button>
                            </div>
                        </form>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="px-6 py-5">
                            <p className="text-slate-600">
                                Tem certeza que deseja excluir esta subcategoria? Os valores registrados nela serão perdidos.
                            </p>
                            <p className="text-xs text-slate-400 mt-2">
                                (Lembre-se de salvar as alterações na tela principal após excluir)
                            </p>
                        </div>
                        <div className="px-6 py-4 bg-slate-50 flex justify-end space-x-3">
                            <button 
